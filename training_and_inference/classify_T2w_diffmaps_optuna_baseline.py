...
class ObjectiveOptunaBaseline(object):
    def __init__(self,
                 nb_output_classes,
                 network,
                 device,
                 ...
                 df_rows_automatic_labels_path="",
                 split_added_data_across_folds=False):
        ...
        # since this is the baseline, by default pretrain_path == "", feature_extracting == False, automatically_annotated_data_path == "",
        # df_rows_automatic_labels_path == "", split_added_data_across_folds == False
        self.pretrain_path = pretrain_path
        self.feature_extracting = feature_extracting
        self.automatically_annotated_data_path = automatically_annotated_data_path
        self.df_rows_automatic_labels_path = df_rows_automatic_labels_path
    ...
    def __call__(self, trial):
        ...
        dict_params = {'learning_rate_optuna': learning_rate_optuna,
        ...
        params_to_update = set_parameter_requires_grad(model,
                                                       ...
                                                       self.network,
                                                       self.fc_nodes_vgg,
                                                       feature_extracting=self.feature_extracting,
        ...
        if self.automatically_annotated_data_path and not self.pretrain_path:
            print("\nLength x_int_train before adding AAD: {}".format(len(x_int_train)))
            x_int_train, y_int_train = add_automatically_annotated_data(self.automatically_annotated_data_path,
                                                                        x_int_train,
                                                                        y_int_train,
                                                                        self.df_rows_automatic_labels_path,
                                                                        self.ext_cv_folds,
        ...
        int_train_ds = Dataset(data=int_train_files, transform=self.train_transforms)
        ...
        val_ds = Dataset(data=val_files, transform=self.val_transforms)
...
def classification_t2_volumes_difference_optuna_baseline(df_dates_and_labels_had: str,
                                                         input_dir: str,
                                                         batch_size: int,
                                                         nb_workers: int,
                                                         ext_cv_folds: int,
    ...
    for ext_train_subs_idxs, test_subs_idxs in kf_ext.split(all_subs):
        ...
        if external_cv_fold_counter == fold_to_do:
            # ---------------------------------------- OPTUNA HYPERTUNING -----------------------------------
            # we include inside ObjectiveOptunaBaseline all the code that contains the parameters to tune
            objective = ObjectiveOptunaBaseline(nb_output_classes, network, device, external_cv_fold_counter, ext_cv_folds, all_subs,
                                                ext_train_subs_idxs, test_subs_idxs, volume_differences_t2, classification_labels, out_dir, percentage_val_subs,
                                                train_transforms, nb_workers, val_transforms, nb_epochs, loss_function, date, val_interval,
                                                patience, batch_size, conv_filters_vgg, fc_nodes_vgg, legend_label)
